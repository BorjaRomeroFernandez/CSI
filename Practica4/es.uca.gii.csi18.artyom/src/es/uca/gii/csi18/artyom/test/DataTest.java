...
class DataTest {
	...
	public void testTableAccess() throws Exception {
		...
		try {
			con = Data.Connection();
			rs = con.createStatement().executeQuery("SELECT Id, Nombre, HorasGuardia FROM soldado;");

			while (rs.next()) {
				System.out.println(rs.getInt("Id") + " " + rs.getString("Nombre") + " " + rs.getInt("HorasGuardia"));
				++i;
			}
			rs.close();
