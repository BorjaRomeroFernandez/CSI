...
class DataTest {
	...
	/**
	 * Method to prove that the connection was successful
	 * 
	 * @throws Exception
	 */
	@Test
	...
	public void testTableAccess() throws Exception {
		...
		try {
			con = Data.Connection();
			rs = con.createStatement().executeQuery("SELECT Id, Id_GunType, Name, GuardHours FROM soldier;");

			while (rs.next()) {
				System.out.println(rs.getInt("Id") + " " +rs.getInt("Id_GunType") + " " + rs.getString("Name") + " " + rs.getInt("GuardHours"));
				++i;
			}
			rs.close();
			rs = con.createStatement().executeQuery("SELECT COUNT(*) FROM soldier;");
			rs.next();
	...
	/**
	 * Method to test Data's String2Sql method
	 */
	@Test
	...
	/**
 	* Method to test Data's Boolean2Sql method
 	*/	
	@Test
